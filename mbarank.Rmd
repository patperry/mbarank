...
We don't have much insight into the different psychologies of business school
deans and corporate recruiters, but empirically, "Peer Assessment" and
"Recruiter Assessment" are strongly correlated with each other (correlation
`r round(cor(school$peer, school$recruit), 2)`) and it seems to us like these
numbers are two different measurements of the same attribute. To combine both
into a single "Combined Assessment Score", we standardize each to have the
same standard deviation and take the average of the two numbers. To make the
result more interpretable, we transform back to the original 1-5 scale;
...
for (j in terms) {
    cat(ifelse(coef[[j]] < 0, " - ", " + "))
    cat(round(abs(coef[[j]]), terms_digits[[j]]))
...
The following table shows the rank and score for the top 30 schools as
...
write.csv(school[, cols], file = "Final Rankings.csv",
          row.names = FALSE)
